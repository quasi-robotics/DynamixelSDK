...
PortHandlerLinuxExt::PortHandlerLinuxExt(const char *port_name)
  : socket_fd_(-1),
...
bool PortHandlerLinuxExt::openPort()
{
  return setBaudRate(baudrate_);
}

void PortHandlerLinuxExt::closePort()
{
...
void PortHandlerLinuxExt::clearPort()
{
  //std::lock_guard<std::mutex> lock(mutex_);
  tcflush(socket_fd_, TCIFLUSH);
}

void PortHandlerLinuxExt::setPortName(const char *port_name)
{
  strcpy(port_name_, port_name);
}

char *PortHandlerLinuxExt::getPortName()
{
  return port_name_;
}

// TODO: baud number ??
bool PortHandlerLinuxExt::setBaudRate(const int baudrate)
{
...
int PortHandlerLinuxExt::getBaudRate()
{
  return baudrate_;
}

int PortHandlerLinuxExt::getBytesAvailable()
{
  //std::lock_guard<std::mutex> lock(mutex_);
  int bytes_available;
...
int PortHandlerLinuxExt::readPort(uint8_t *packet, int length)
{
  //std::lock_guard<std::mutex> lock(mutex_);
  return read(socket_fd_, packet, length);
}

int PortHandlerLinuxExt::writePort(uint8_t *packet, int length)
{
  //std::lock_guard<std::mutex> lock(mutex_);
  return write(socket_fd_, packet, length);
}

void PortHandlerLinuxExt::setPacketTimeout(uint16_t packet_length)
{
  //std::lock_guard<std::mutex> lock(mutex_);
...
void PortHandlerLinuxExt::setPacketTimeout(double msec)
{
  //std::lock_guard<std::mutex> lock(mutex_);
  packet_start_time_  = getCurrentTime();
...
bool PortHandlerLinuxExt::isPacketTimeout()
{
  //std::lock_guard<std::mutex> lock(mutex_);
...
double PortHandlerLinuxExt::getCurrentTime()
{
	struct timespec tv;
...
double PortHandlerLinuxExt::getTimeSinceStart()
{
  double time;
...
bool PortHandlerLinuxExt::setupPort(int cflag_baud)
{
...
bool PortHandlerLinuxExt::setCustomBaudrate(int speed)
{
...
int PortHandlerLinuxExt::getCFlagBaud(int baudrate)
{
  switch(baudrate)
  {
    case 9600:
...
bool PortHandlerLinuxExt::waitForData(double timeout) {
  ...
  int ret = select(socket_fd_+1, &fds, NULL, NULL, &tp);
  if (ret == -1) {
